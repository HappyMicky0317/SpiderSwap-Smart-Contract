...
import './SpiderSwapV2ERC20.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';
import './interfaces/IERC20.sol';
import './interfaces/ISpiderSwapV2Factory.sol';
import './interfaces/ISpiderSwapV2Callee.sol';
...
contract SpiderSwapV2Pair is SpiderSwapV2ERC20 {
   ...
   function takeRemoveLiquidityFee(address _token0, address _token1, uint256 _amount0, uint256 _amount1) internal returns(uint256, uint256){
        ...
        _safeTransfer(_token0, ISpiderSwapV2Factory(factory).DEADADDRESS(), bankFee0[1]);
        _safeTransfer(_token1, ISpiderSwapV2Factory(factory).DEADADDRESS(), bankFee1[1]);

        _safeTransfer(_token0, ISpiderSwapV2Factory(factory).global(), bankFee0[2]);
        _safeTransfer(_token1, ISpiderSwapV2Factory(factory).global(), bankFee1[2]);

        _safeTransfer(_token0, ISpiderSwapV2Factory(factory).roulette(), bankFee0[3]);
        _safeTransfer(_token1, ISpiderSwapV2Factory(factory).roulette(), bankFee1[3]);
